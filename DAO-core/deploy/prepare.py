...
"""Prepare scripts for DAO deploy"""
...
class TestDeployContext:
    ...
    def compile_contract(self, contract_name):
        if self.args.no_limits and contract_name == "DAO.sol":
            ...
            contract_path = os.path.join(self.args.contracts_dir, contract_name)
        print("    Compiling {}...".format(contract_path))
        data = subprocess.check_output(
            [self.args.solc, contract_path, "--optimize", "--combined-json", "abi,bin"]
        )
    ...
    def cleanup(self):
        try:
            ...
            rm_file(os.path.join(self.args.contracts_dir, "TokenCreationCopy.sol"))
        except:
            pass
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=int,
        default=3600,
        help=("Minumum number of seconds that a generic proposal can have"),
    )
    p.add_argument(
        "--min-split-debate",
        type=int,
        default=3600,
        help=("Minumum number of seconds that a split proposal can have"),
    )
    p.add_argument(
        "--offer-contractor",
        default="0x08144824954c65b12f68b75072488e634ac4e67a",  # Griff testnet
    ...
    with open("prepare.js", "w") as f:
        ...
        f.write("creator_abi = {};\n".format(comp["contracts"]["DAO_Creator"]["abi"]))
        f.write("creator_bin = '{}';\n".format(comp["contracts"]["DAO_Creator"]["bin"]))
        f.write("offer_abi = {};\n".format(comp2["contracts"]["SampleOffer"]["abi"]))
        f.write("offer_bin = '{}';\n".format(comp2["contracts"]["SampleOffer"]["bin"]))
        f.write("seconds_from_now = {};\n".format(args.creation_duration_mins * 60))
        f.write('curator = "{}";\n'.format(args.curator))
        f.write(
            "default_proposal_deposit = {};\n".format(args.default_proposal_deposit)
        )
        f.write('contractor = "{}";\n'.format(args.offer_contractor))
        f.write("offer_total_costs = {};\n".format(to_wei(args.offer_total_costs)))
        f.write("offer_onetime_costs = {};\n".format(to_wei(args.offer_onetime_costs)))
        f.write(
        ...
        f.write(
            "offer_client_dao_address = '{}';\n".format(args.offer_client_dao_address)
        )
